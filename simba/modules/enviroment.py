...
class SchellingSim(Env):
    def __init__(self):
        super(SchellingSim, self).__init__()
    ...
    def step(self, action, prevState):
        ...
        self.i += 1
        node_attr = prevState.set_index('index').to_dict('index')
        ...
        for mover in movers:
            ...
            income_pref = [n for n in race_pref if (self.graph.nodes[n]["average_income"] - 5000 < mover_income < self.graph.nodes[n]["average_income"] + 10000)]
        ...
        self.df = pd.DataFrame.from_dict(dict(self.graph.nodes(data=True)), orient='index')
    ...
    def reset(self):
        ...
        geofr = gpd.read_file("../data/tl_2017_51_tabblock10.zip")
        ...
        housefr = pd.read_csv("../data/household.csv", usecols = ['admin2','admin3', 'hid', 'hh_income', 'residence_longitude', 'residence_latitude'])
        ...
        personfr = pd.read_csv("../data/person.csv", usecols = ['hid', 'race'])
        print(personfr.info())
        ...
        for index, row in synthfr.iterrows():
            self.graph.add_node(row['lid'], hid = row['hid'], race = row['hh_race'], income = row['hh_income'], geometry = row['geometry'])
